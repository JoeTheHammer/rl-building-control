...
- **Completed Experiment Suites**: Shows experiments that have completed (finished or aborted).
...
Once you have scheduled an experiment suite, you can see additional details (by clicking on `Show details`) of the individual experiments used in this experiment suite including their `.yaml` configuration files. When investigating `.yaml` files, you can use the corresponding buttons on the bottom to navigate to the corresponding configurator page and change them:
...
Once an experiment has completed it will appear in the Completed Experiment Suites section.

![alt text](images/execution_5.png)

You can now archive the experiment, such that it is not on your dashboard anymore (but still accessible in the section `Archive`). The TensorBoard logs are still available and can be viewed in [TensorBoard](https://www.tensorflow.org/tensorboard). `Show results` opens the `Data Analysis` page with the corresponding results, which is described in more detail [here](03-data-analysis.md).

After clicking on `Show Details` you see options to open the individual `.yaml` configuration files or the logs.

# Configuration execution without GUI
