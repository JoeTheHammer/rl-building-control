...
To access the frontend, start the application via Docker (see README in project root) and access `http://localhost:5173/`. Navigate via the top bar to `Data Analysis` or click on `Show Results` in the `Experiments` dashboard ([more information](02-running-experiments.md)).

## Data Analysis in GUI

You can open experiment data via the button `Load Data`. For each experiment in the experiment suite, you will get an overview of the results:

![alt text](images/data-analysis-1.png)

At the top of the card, you can see some information for the experiment, like the config `.yaml` files used or the total reward collected during evaluation.
...
- **Reproduce**: Creates a new experiment in the `Experiments` section, with exactly the same configuration and environment data (building model, weather data). This allows you to reproduce the experiment.
...
- **Actions**: Allows you to investigate the taken action per timestep for each actuator.
- **States**: Allows you to investigate the measured value for each timestep for each variable in the state space.
...
This screenshot shows the same data as in the screenshot before but this time presented as a bar chart with averaging every 200 timesteps.
