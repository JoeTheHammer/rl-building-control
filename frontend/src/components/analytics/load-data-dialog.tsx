...
const LoadDataDialog: React.FC<LoadDataDialogProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[800px] max-w-3xl overflow-hidden">
        <DialogHeader>
          <DialogTitle>Select Experiment Suite</DialogTitle>
        </DialogHeader>

        <div className="flex max-h-[800px] max-w-3xl flex-col gap-4 overflow-y-auto">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {suites.map((suite) => (
                <button
                  ...
                  className="bg-card hover:border-primary focus-visible:ring-ring flex flex-col items-start gap-2 rounded-lg border p-4 text-left transition focus-visible:ring-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-60"
                >
